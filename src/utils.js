...
export const returnIconClassName = (id, dayOrNight) => {
  let icon = weatherIcons[id].icon
  if (dayOrNight === 'd') {
    var iconClassName = `wi wi-day-${icon}`;
    return iconClassName

  }
  else if (dayOrNight === 'n') {
    // the night version of the icon for id 800 isn't available in weatherIcons
    if (id === 800) {
      var iconClassName = "wi wi-night-clear"
      return iconClassName
    }
    var iconClassName = `wi wi-night-${icon}`;
    return iconClassName
  }
}
